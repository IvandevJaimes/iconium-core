...
const cleanQuery = (str: string): string => str.toLowerCase().replace(/[^a-z0-9]/g, "");
...
const getIconData = (index: number): { title: string; slug: string; hex: string } => {
...
export const searchIcon = (
  ...
  if (cleanQ.length <= 2) {
    const abbrevs: Record<string, string> = { go: "go", rs: "rust", cpp: "cplusplus" };
  ...
  const scored = ICON_INDEX.map((iconData, index) => {
    ...
    for (let i = 0; i < cleanQ.length; i++) {
      ...
      if (idx === -1) { hasAllChars = false; break; }
      if (i === 0 && idx === 0) firstCharMatch = true;
      lastMatchIdx = idx;
    }

    if (hasAllChars) {
      return { index, score: firstCharMatch ? 0.88 : 0.75 };
    }

    return { index, score: similarity(cleanQ, nameClean) };
  });
